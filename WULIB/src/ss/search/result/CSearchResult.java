...
import ss.search.catalog.CSearchAdvance;
...
public class CSearchResult extends Activity {
	...
	private void bindWidget2() {
		try {
			...
			if (className.equals(CSearchByKeyword.class + "")
					&& className.equals(SearchBySpeech.class + "")) {
				searchByKeyword();
			} else if (className.equals(CSearchAdvance.class + "")) {
				searchAdvance();
			} else {
				searchByKeyword();
			}
	...
	private void searchByKeyword() throws Exception {
		if (component.isOnline(this)) {
			...
			// component.showYesNoDialogBox(this, "test", "sf = " + searchFor1
			// + "in = " + searchIn1);

			// String txt = "";
			// txt += "sf = " + searchFor1 + " in = " + searchIn1;

			if (intent.getBooleanExtra("filter", false)) {
				String mtType = intent.getStringExtra("mtType");
				String collection = intent.getStringExtra("collection");
				String location = intent.getStringExtra("location");
				String language = intent.getStringExtra("language");
				String beginYear = intent.getStringExtra("beginYear");
				String endYear = intent.getStringExtra("endYear");

				if (!mtType.equals(getResources().getStringArray(
						R.array.spn_csk_itemtype)[0])) {
					ldata.add(new BasicNameValuePair("mtType", mtType));
					// txt += " mtype = " + mtType;
				}

				if (!collection.equals(getResources().getStringArray(
						R.array.spn_csk_collection)[0])) {
					ldata.add(new BasicNameValuePair("collection", collection));
					// txt += " collection = " + collection;
				}
				if (!location.equals(getResources().getStringArray(
						R.array.spn_csk_location)[0])) {
					ldata.add(new BasicNameValuePair("location", location));
					// txt += " location = " + location;
				}

				if (!language.equals(getResources().getStringArray(
						R.array.spn_language)[0])) {
					ldata.add(new BasicNameValuePair("language", language));
					// txt += " language = " + language;
				}

				if (!beginYear.equals("ALL")) {
					ldata.add(new BasicNameValuePair("bYear", beginYear));
					// txt += " beginYear = " + beginYear;
				}

				if (!endYear.equals("ALL")) {
					ldata.add(new BasicNameValuePair("eYear", endYear));
					// txt += " endYear = " + endYear;
				}
			...
			bindListViewItem(url, ldata);
			// component.showYesNoDialogBox(this, "test", txt);
		} else {
	...
	public void bindListViewItem(String url, List<NameValuePair> ldata) {
		...
		// String[] data = component.getTitleBooks(component.books);
		// bindLVItem();
	}
	...
	private void bindLVItem() {
		try {
			...
			component.showMsg(
					this,
					"in CSearchResult.java on bindLVItem function "
							+ e.toString());
		}
	...
	protected void goToDetail() {
		Intent in = new Intent(CSearchResult.this,
	...
	public boolean onContextItemSelected(MenuItem item) {
		...
		case R.id.menu_it_sr_addfavorite:
			component.addFavorite(this, positionSelected, false, "C");
			bindWidget2();
			bindLVItem();
			break;
		}
